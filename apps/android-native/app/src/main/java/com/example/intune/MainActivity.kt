...
class MainActivity : ComponentActivity() {
    private var notificationNudge: NudgeResponse? by androidx.compose.runtime.mutableStateOf(null)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            IntuneTheme {
                GoalAwareApp(GoalsRepository(applicationContext), createNudgeApi(), rememberCoroutineScope(), notificationNudge) {
    ...
    private fun android.content.Intent.toNudge(): NudgeResponse? = getStringExtra(EXTRA_NUDGE_MESSAGE)?.let {
